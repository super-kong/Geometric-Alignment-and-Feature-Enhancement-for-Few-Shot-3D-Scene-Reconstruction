...
def render(viewpoint_camera, pc, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0,
    ...
    elif len(confidence.shape) == 2 and confidence.shape[1] != 1:
        confidence = confidence.reshape(-1, 1)
    # 确保长度匹配当前高斯数
    if confidence.shape[0] != pc.get_xyz.shape[0]:
        confidence = confidence[:pc.get_xyz.shape[0]] if confidence.shape[0] > pc.get_xyz.shape[0] else torch.cat([
            confidence, torch.ones((pc.get_xyz.shape[0] - confidence.shape[0], 1), device=confidence.device)
        ])
