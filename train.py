...
from utils.scheduler import build_scheduler, CosineScheduler
...
def training(dataset, opt, pipe, abla, args):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if args.use_multi_scale_depth:
            scales = [0.25, 0.5, 1.0]
            depth_loss = 0.0

            for scale in scales:

                h = rendered_depth.shape[-2]
                w = rendered_depth.shape[-1]
                ...
                if scale < 1.0:
                    ...
                    scaled_rendered = F.interpolate(
                        rendered_depth.unsqueeze(0).unsqueeze(0),
                        size=(new_h, new_w),
                        mode='bilinear',
                        align_corners=False
                    ).squeeze()
                    scaled_midas = F.interpolate(
                    ...
                    scaled_rendered = rendered_depth
                    scaled_midas = midas_depth

                scaled_rendered = scaled_rendered.reshape(-1, 1)
                scaled_midas = scaled_midas.reshape(-1, 1)

                scale_loss = min(
                    (1 - pearson_corrcoef(-scaled_midas, scaled_rendered)),
                    (1 - pearson_corrcoef(1 / (scaled_midas + 200.), scaled_rendered))
                )
                depth_loss += scale_loss * (1.0 / len(scales))
        else:
            depth_loss = min(
        ...
        # loss += gaussians.regularization_loss(lambda_opacity=0.01)
        if args.use_multi_view_consistency:
            all_train_cameras = scene.getTrainCameras()

            import random
            if len(all_train_cameras) >= 2:
                num_sample_cams = min(4, len(all_train_cameras))
                cameras = random.sample(all_train_cameras, num_sample_cams)
            else:
                cameras = all_train_cameras
            loss += gaussians.multi_view_consistency_loss(cameras, lambda_mv=0.05)
        loss.backward()

        if iteration > 1000 and iteration % 10 == 0:
            gaussians.apply_depth_scale_constraint(iteration)
        ...
        with torch.no_grad():
            ...
            if args.use_dd_drop:
                print("Using DD drop")
                camera_pos = viewpoint_cam.camera_center
                camera_pos = torch.tensor(camera_pos, dtype=torch.float32, device="cuda")
                if iteration > 500 and iteration > opt.densify_until_iter:
                    gaussians.apply_dd_drop(camera_pos, iteration)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_litept', action='store_true', default=False, help='Enable LitePT feature fusion in Gaussian Model (default: False)')
